...
function task1(cardInfo,callback){ 
  getBoard(function(boardId){
    getLists(boardId.id,function(listsId){
      let cardDataId=listsId.filter(obj=>obj.id==cardInfo)
      //console.log(carDataId)
      getCards(cardDataId[0].id,function(card){
      callback(card)
      })
    })
  })
}
const cardListsTask1=(card)=>{
  console.log(card)
}
// task1('qwsa221',cardListsTask1)

// Task 2 board -> lists -> cards for list qwsa221 and cards for list jwkh245 simultaneously

function task2(cardInfo1,cardInfo2,callback){
  getBoard(function(boardId){
    getLists(boardId.id,function(listsId){
      let cardsId=listsId.filter(obj=>obj.id==cardInfo1||obj.id==cardInfo2)
    //  console.log(cardsId)
      cardsId.forEach(obj=>getCards(obj.id,function(card){
        callback(card)
        //console.log(card)
      }))
    })
...
const cardListsTask2=(card)=>{
  console.log(card)
}

// task2('qwsa221','jwkh245',cardListsTask2)
...
const cardListsTask3=(card)=>{
  console.log(card)
}
task3(cardListsTask3)